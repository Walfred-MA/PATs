...
void CramReader::Load()
{
    ...
    if (strlen(filepath) >= 5 && std::string(filepath).substr(strlen(filepath) - 5) == ".cram")
    {
        ...
        if (refpath.empty())
        {
            std::cerr << "WARNING: CRAM input " << filepath
                      << " was opened without a reference FASTA. "
                      << "If decoded bases are missing or all counts are zero, rerun with --reference."
                      << std::endl;
        }
    }
