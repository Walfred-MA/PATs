...
extern bool ifstrand;
bool ifstrand = 0;

template <int dictsize>
void run(std::vector<std::string> &inputfiles, std::vector<std::string>& targetfiles, std::vector<std::string> &outputfiles, std::vector<std::string> &prefixes, const int kmer_size, const int nthreads, const int mode, const int cutoff, const std::string &reference_path)
{
    /*
    if (mode == 3)
    {
    ...
    else if (mode == 0)
    {
        ...
        kmer_counter<dictsize> counter(kmer_size, reference_path);
...
int main(int argc, const char * argv[]) {
    ...
    for (int i = 1; i < argc ; i++)
    {
        ...
        else if (strcmp(Argument, "-r")==0 or strcmp(Argument, "--reference")==0)
        {
        ...
        else if (strcmp(Argument, "-S")==0 or strcmp(Argument, "--Strand")==0)
        {   
            ifstrand = 1;
        }
    ...
    run<32>(inputfiles, targetfiles, outputfiles, prefixes, kmer_size, nthreads, mode, cutoff, reference_path);
